...
/// \brief Creates coefficient vector of a quintic spline defined on the
/// interval [tBegin, tEnd]. It follows the equation x(t) = a + b(t - t_min_) +
/// c(t - t_min_)^2 + d(t - t_min_)^3 + e(t - t_min_)^4  + f(t - t_min_)^5
///
template <typename Point, typename T_Point>
