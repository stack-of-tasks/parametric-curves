...
Polynomial<Numeric, Dim, Point> create_cubic(Point const& a, Point const& b, Point const& c, Point const& d,
...
template <typename Numeric, Eigen::Index Dim, typename Point, typename T_Point>
Polynomial<Numeric, Dim, Point> create_quintic(Point const& a, Point const& b, Point const& c, Point const& d,
                                               Point const& e, Point const& f, const Numeric min, const Numeric max) {
...
template <typename Numeric = double, Eigen::Index Dim = Eigen::Dynamic,
          typename Point = Eigen::Matrix<Numeric, Dim, 1>, typename SplineBase = Polynomial<Numeric, Dim, Point> >
struct Spline : public AbstractCurve<Numeric, Point> {
 ...
 public:
  ...
  Spline(const t_spline_t& subSplines)
      : curve_abc_t(subSplines.front().tmin(), subSplines.back().tmax()), subSplines_(subSplines) {}
 ...
 public:
  ...
  void createSplineFromWayPoints(In wayPointsBegin, In wayPointsEnd) {
    ...
    Numeric min = (*it).first;
    Point a_ =
        a.row(size - 1) - b.row(size - 1) * min + c.row(size - 1) * min * min - d.row(size - 1) * min * min * min;
  ...
  void createSplineFromWayPointsConstr(In wayPointsBegin, In wayPointsEnd, const spline_constraints& constraints) {
    ...
    ++next;
    for (std::size_t i(0); next != end; ++next, ++it, ++i) compute_one_spline<In>(it, next, cons, subSplines_);
 ...
 private:
  ...
  void compute_one_spline(In wayPointsBegin, In wayPointsNext, spline_constraints& constraints,
    ...
    Point d_ = d0;
    subSplines.push_back(create_cubic<Numeric, Dim, Point, t_point_t>(a_, b_, c_, d_, init_t, end_t));
  ...
  void compute_end_spline(In wayPointsBegin, In wayPointsNext, spline_constraints& constraints,
    ...
    subSplines.push_back(create_quintic<Numeric, Dim, Point, t_point_t>(a_, b_, c_, d_, e_, f_, init_t, end_t));
  }
