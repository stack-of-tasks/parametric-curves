...
Bezier* wrapBezierConstructorConstraintsTemplate(const PointList& array, const CurveConstraints& constraints, const
real lb = 0., const real ub =1.)
{
    T_Point asVector = vectorFromEigenArray<PointList, T_Point>(array);
...
{
    return wrapBezierConstructorConstraintsTemplate<bezier_t, point_list_t, t_point_t, curve_constraints_t>(array,
constraints) ;
}
bezier_t* wrapBezierConstructorBoundsConstraints(const point_list_t& array, const curve_constraints_t& constraints,
const real lb, const real ub)
{
    return wrapBezierConstructorConstraintsTemplate<bezier_t, point_list_t, t_point_t, curve_constraints_t>(array,
constraints, lb, ub) ;
}
...
{
    return wrapBezierConstructorConstraintsTemplate<bezier6_t, point_list6_t, t_point6_t, curve_constraints6_t>(array,
constraints) ;
}
...
polynom_t* wrapSplineConstructor(const coeff_t& array) { return new polynom_t(array, 0., 1.); }

t_waypoint_t getWayPoints(const coeff_t& array, const time_waypoints_t& time_wp) {
  t_waypoint_t res;
  for (int i = 0; i < array.cols(); ++i) res.push_back(std::make_pair(time_wp(i), array.col(i)));
  return res;
}
...
Eigen::Matrix<real, Eigen::Dynamic, Eigen::Dynamic> wayPointsToList(const BezierType& self) {
  ...
  int col = 0;
  for (cit_point cit = wps.begin(); cit != wps.end(); ++cit, ++col) res.block<dim, 1>(0, col) = *cit;
  return res;
}
void spline_from_waypoints(spline_t& self, const coeff_t& array, const time_waypoints_t& time_wp) {
...
spline_t* wrapExactCubicConstructorPolySequence(const bp::list& list_polynomials, const time_vector_t& time_vector) {
  typedef std::vector<t_point_t, Eigen::aligned_allocator<t_point_t> > t3d_poly_coeffs_vector_t;
  ...
  for (int i = 0; i < len(list_polynomials); ++i) {
    subSplines.push_back(polynom_t(bp::extract<coeff_t>(list_polynomials[i]), time_vector[i], time_vector[i + 1]));
...
void spline_from_waypoints_constr(spline_t& self, const coeff_t& array, const time_waypoints_t& time_wp,
...
BOOST_PYTHON_MODULE(libparametric_curves_pywrap) {
  ...
  */
  ...
  class_<spline_t>("spline", no_init)
      ...
      .def("create_spline_from_waypoints", &spline_from_waypoints, boost::python::args("waypoints", "time vector"),
  ...
  class_<force_t>("forcecurve", no_init)
      ...
      .def("set_motion_vector", &force_t::setMotionVector, boost::python::args("motionvector"),
